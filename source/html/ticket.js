...
document.getElementById('backToOverviewBtn').onclick = function(e) {
	e.preventDefault();
...
async function fetchTicketDetails() {
	if (!eventId || !ticketId) {
		document.getElementById('ticketDetails').innerHTML = '<div class="text-red-500">No event or ticket ID provided.</div>';
		return;
	}
	try {
		const response = await fetch(`http://localhost:8000/ticket/${eventId}/${ticketId}`);
		if (!response.ok) throw new Error('Not found');
		const data = await response.json();
		// Only show date part (YYYY-MM-DD) for event_date
		let eventDateStr = data.event_date ? String(data.event_date).split('T')[0] : '';
		...
		if (Array.isArray(data.tickets) && data.tickets.length > 0) {
			ticketTypesTable = `<div class="mt-4">
				...
				<table class="min-w-full mt-2 mb-2 bg-slate-50 rounded border">
					...
					<tbody>
						${data.tickets.map((tt, idx) => {
							const presenceText = tt.status_presence === 'present' ? 'ja' : 'nee';
							return `
		...
		document.getElementById('ticketDetails').innerHTML = '<div class="text-red-500">Failed to load ticket details.</div>';
	}
}
fetchTicketDetails();

// Overlay logic
window.showPresenceOverlay = function(idx) {
	...
	document.getElementById('presenceForm').onsubmit = async function(e) {
		e.preventDefault();
		const form = e.target;
		const newStatus = form.status_presence.value;
		if (window._ticketData.tickets[idx].status_presence !== newStatus) {
			const confirmed = confirm('Weet je zeker dat je de aanwezigheid wilt aanpassen?');
			if (!confirmed) return;
		}
		...
		try {
			await fetch(`http://localhost:8000/ticket/${eventId}/${ticketId}/${newStatus}`, {
				method: 'GET',
...
window.hidePresenceOverlay = function() {
